...
ifeq ("$(INSTRUMENT)","")
	...
	$(PYTHON) -m songbook_tools insertchords --chords-per-line $(CHORDS_PER_LINE) $< > $@
endif

build/$(SONGBOOK)/songlist.tex: $(BUILT_SONGS) songbook_tools/*.py
	cd build/$(SONGBOOK) && PYTHONPATH=../.. $(PYTHON) -m songbook_tools makesonglist songs > ../../$@

build/$(SONGBOOK)/buildinfo.tex: songbook_tools/*.py
	mkdir -vp build/$(SONGBOOK)
	$(PYTHON) -m songbook_tools makebuildinfo > $@
...
build/$(SONGBOOK)/chords.tex: songbook_tools/*.py chords/*.ini
	mkdir -vp build/$(SONGBOOK)
ifneq ($(INSTRUMENT),)
	$(PYTHON) -m songbook_tools makechords chords/$(INSTRUMENT).ini > $@
endif
